    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={exportCSV} title="Export product data as CSV" className="flex items-center gap-2 px-4 py-2.5 bg-[#0a0514] hover:bg-white/5 border border-white/10 rounded-lg text-sm text-gray-300 transition-colors">
            <Download className="w-4 h-4" /> Export CSV
          </button>
          <button onClick={openCreate} title="Add a new product" className="flex items-center gap-2 px-4 py-2.5 bg-[#9a02d0] hover:bg-[#7a01a8] rounded-lg text-sm text-white transition-colors">
      ...
      <div className="bg-[#150f24] border border-white/10 rounded-xl overflow-hidden">
                      ...
                      <td className="px-4 py-3">
                        {pVariants.length > 0 ? (
                          <button
                            ...
                            title={isExpanded ? 'Collapse variants' : 'View packaging variants'}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-white/10">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} title="Previous page" className="p-2 hover:bg-white/5 rounded-lg disabled:opacity-30"><ChevronLeft className="w-4 h-4" /></button>
            <span className="text-sm text-gray-400">Page {page + 1} of {totalPages || 1}</span>
            <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} title="Next page" className="p-2 hover:bg-white/5 rounded-lg disabled:opacity-30"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#150f24] border border-white/10 rounded-xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
              ...
              {activeTab === 'details' ? (
                ...
                <div className="space-y-4">
                    ...
                    <div key={v.id} className="bg-[#0a0514] border border-white/10 rounded-lg p-4">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-1 ml-3">
                          <button onClick={() => openVariantForm(v)} title="Edit this variant" className="p-1.5 hover:bg-white/5 rounded-lg text-gray-400 hover:text-[#44f80c]"><Pencil className="w-3.5 h-3.5" /></button>
                  ...
                  {showVariantForm && (
                    <div className="bg-[#0a0514] border border-[#9a02d0]/30 rounded-lg p-4 space-y-3">
                      ...
                      <div className="flex gap-2 pt-2">
                        <button onClick={() => { setShowVariantForm(false); setEditingVariantId(null) }} title="Cancel variant changes"
                          className="px-3 py-2 bg-[#150f24] hover:bg-white/5 border border-white/10 rounded-lg text-xs text-gray-300">Cancel</button>
                        <button onClick={saveVariant} title={editingVariantId ? 'Save variant changes' : 'Create new variant'}
                  ...
                  {!showVariantForm && editingProduct && (
                    <button onClick={() => openVariantForm()} title="Add a new packaging variant"
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-white/10 flex-shrink-0">
              ...
              {activeTab === 'details' && (
                <button onClick={handleSave} title={editingProduct ? 'Save product changes' : 'Create new product'} className="px-4 py-2.5 bg-[#9a02d0] hover:bg-[#7a01a8] rounded-lg text-sm text-white">
