...
export function CommissionsPage() {
  ...
  const fetchSettings = async () => {
    try {
      const { data } = await supabase.from('commission_settings').select('id, rep_rate, manager_override_rate').maybeSingle()
      if (data) {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      if (settingsId) {
        ...
        // Insert new row (id auto-generated by database)
        const { data, error } = await supabase.from('commission_settings').insert({
          rep_rate: settings.rep_rate,
          manager_override_rate: settings.manager_override_rate,
        }).select('id').single()
        if (error) throw error
  ...
  const fetchDirectory = async () => {
    ...
    try {
      ...
      // Fetch all commission entries for calculations
      const { data: allEntries } = await supabase
        .from('commission_entries')
        .select('rep_id, manager_id, rep_earnings, manager_earnings, status')
      ...
      ;(repsData || []).forEach((r: any) => {
        const repEntries = (allEntries || []).filter((e: any) => e.rep_id === r.id)
        ...
        repMap.set(r.id, {
          ...
          commission: repEntries.reduce((sum: number, e: any) => sum + (e.rep_earnings || 0), 0),
          pending: repEntries.filter((e: any) => e.status === 'accrued').reduce((sum: number, e: any) => sum + (e.rep_earnings || 0), 0),
        })
      })
      ...
      ;(managersData || []).forEach((m: any) => {
        ...
        const mgrEntries = (allEntries || []).filter((e: any) => e.manager_id === m.id)
        mgrMap.set(m.id, {
          id: m.id,
          name: m.business_name || m.email,
          email: m.email,
          team_size: teamReps.length,
          overrides: mgrEntries.reduce((sum: number, e: any) => sum + (e.manager_earnings || 0), 0),
          pending: mgrEntries.filter((e: any) => e.status === 'accrued').reduce((sum: number, e: any) => sum + (e.manager_earnings || 0), 0),
        })
      })
